...
# -*- coding: utf-8 -*-
"""
基于火山引擎豆包大模型的智能POI解析器（修复版）
直接调用火山引擎的Doubao-Seed-1.6模型
"""
...
class DoubanAIParser:
    ...
    def __init__(self, api_key: str = None):
        """初始化火山引擎豆包API解析器"""
        self.api_key = api_key or "your_volcengine_api_key_here"
        # 火山引擎豆包API地址
    ...
    def parse_note(self, text: str, url: str = "") -> Optional[Dict]:
        ...
        try:
            self.logger.info("开始使用火山引擎豆包大模型解析笔记")
            ...
            # 构建请求数据 - 火山引擎豆包API格式
            request_data = {
                "model": "doubao-seed-1.6",  # 直接使用火山引擎的Doubao-Seed-1.6模型
                "input": {
                    "messages": [
                        {
                            "role": "user",
